...
func TestNewTraceDecorator(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			base: mockusecases.NewMockSettingsUseCases(gomock.NewController(t)),
		},
		{
...
func TestTraceDecorator_GetSettingsByUserID(t *testing.T) {
	...
	tests := []struct {
		name             string
		userID           uint64
		setupMocks       func(*mocktracing.MockProvider, *mockusecases.MockSettingsUseCases, *mocktracing.MockSpan)
	...
	}{
		{
			...
			setupMocks: func(
				...
				mockBase *mockusecases.MockSettingsUseCases,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockBase := mockusecases.NewMockSettingsUseCases(ctrl)
