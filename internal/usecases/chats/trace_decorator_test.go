...
func TestNewTraceDecorator(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			base: mockusecases.NewMockChatsUseCases(gomock.NewController(t)),
		},
		{
...
func TestTraceDecorator_GetChatMembers(t *testing.T) {
	...
	tests := []struct {
		name            string
		chatID          uint64
		setupMocks      func(*mocktracing.MockProvider, *mockusecases.MockChatsUseCases, *mocktracing.MockSpan)
	...
	}{
		{
			...
			setupMocks: func(
				...
				mockBase *mockusecases.MockChatsUseCases,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockBase := mockusecases.NewMockChatsUseCases(ctrl)
...
func TestTraceDecorator_GetUserChats(t *testing.T) {
	...
	tests := []struct {
		...
		{
			name:   "get user chats with default pagination",
			userID: 1,
			pagination: &domains.Pagination{
		...
		{
			name:   "get user chats with offset",
			userID: 1,
			pagination: &domains.Pagination{
				Limit:  pointers.New[uint64](10),
