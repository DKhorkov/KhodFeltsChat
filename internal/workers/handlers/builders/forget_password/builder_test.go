...
import (
	...
	"github.com/DKhorkov/kfc/internal/workers/handlers/builders/forget_password"
...
func TestNew(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			builder := forget_password.New(tt.useCases, tt.logger)
...
func TestBuilder_MessageHandler(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			message: func() *nats.Msg {
				dto := domains.ForgetPasswordNotificationDTO{UserID: 1}
		...
		},
		{
			...
			message: func() *nats.Msg {
				dto := domains.ForgetPasswordNotificationDTO{UserID: 2}
				data, _ := json.Marshal(dto)
		...
		},
		{
			...
			setupMocks: func(
				...
				mockUseCases.EXPECT().
					SendForgetPasswordMessage(gomock.Any(), uint64(999999)).
					Return(nil)
			},
			message: func() *nats.Msg {
				dto := domains.ForgetPasswordNotificationDTO{UserID: 999999}
				data, _ := json.Marshal(dto)
		...
		},
		{
			...
			setupMocks: func(
				...
				mockUseCases.EXPECT().
					SendForgetPasswordMessage(gomock.Any(), uint64(0)).
			...
			message: func() *nats.Msg {
				dto := domains.ForgetPasswordNotificationDTO{UserID: 0}
				data, _ := json.Marshal(dto)
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			builder := forget_password.New(mockUseCases, mockLogger)
