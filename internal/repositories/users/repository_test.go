...
func (s *RepositoryTestSuite) SetupTest() {
	...
	s.repository = users.New(tx, s.logger)
...
func (s *RepositoryTestSuite) TestGetUserByID_Success() {
...
func (s *RepositoryTestSuite) TestGetUserByID_NotFound() {
...
func (s *RepositoryTestSuite) TestGetUserByID_ZeroID() {
...
func (s *RepositoryTestSuite) TestGetUserByUsername_Success() {
...
func (s *RepositoryTestSuite) TestGetUserByUsername_NotFound() {
...
func (s *RepositoryTestSuite) TestGetUserByUsername_CaseSensitive() {
...
func (s *RepositoryTestSuite) TestGetUserByEmail_Success() {
...
func (s *RepositoryTestSuite) TestGetUserByEmail_NotFound() {
...
func (s *RepositoryTestSuite) TestGetUserByEmail_CaseSensitive() {
...
func (s *RepositoryTestSuite) TestGetUsers_WithoutFilters() {
...
func (s *RepositoryTestSuite) TestGetUsers_WithUsernameFilter() {
...
func (s *RepositoryTestSuite) TestGetUsers_WithPagination() {
...
func (s *RepositoryTestSuite) TestGetUsers_WithFilterAndPagination() {
...
func (s *RepositoryTestSuite) TestGetUsers_EmptyResult() {
...
func (s *RepositoryTestSuite) TestUpdateUser_Success() {
	...
	userData.Username = "updated_john"
	...
	originalUser2, err := s.repository.GetUserByID(s.ctx, 2)
	s.NoError(err)
	s.NotNil(originalUser2)

	userData = *originalUser2
	userData.Username = "updated_jane"
...
func (s *RepositoryTestSuite) TestUpdateUser_NoChanges() {
	// Тест: Обновление полной сущности без изменений — поля должны сохраниться.
	originalUser, err := s.repository.GetUserByID(s.ctx, 1)
	s.NoError(err)
	s.NotNil(originalUser)

	err = s.repository.UpdateUser(s.ctx, *originalUser)
	s.NoError(err)

	user, err := s.repository.GetUserByID(s.ctx, 1)
	s.NoError(err)
	s.NotNil(user)
	s.Equal(originalUser.Username, user.Username)
	s.Equal("john@example.com", user.Email)
	s.True(user.EmailConfirmed)
	s.Equal("$2a$10$hashedpassword1", user.Password)
}

func (s *RepositoryTestSuite) TestUpdateUser_LongUsername() {
...
func (s *RepositoryTestSuite) TestUpdateUser_SameUsername() {
	// Тест: Обновление с тем же именем пользователя
	originalUser, err := s.repository.GetUserByID(s.ctx, 1)
	s.NoError(err)
	s.NotNil(originalUser)

	userData := *originalUser // То же самое имя
...
func (s *RepositoryTestSuite) TestUpdateUser_NotFound() {
...
func (s *RepositoryTestSuite) TestUpdateUser_SpecialCharacters() {
	...
	userData := *originalUser
	userData.Username = "user-name.with.dots_and_underscores"
...
func (s *RepositoryTestSuite) TestUpdateUser_ConcurrentUpdates() {
	// Тест: Проверка, что последнее обновление сохраняется
	originalUser, err := s.repository.GetUserByID(s.ctx, 1)
	s.NoError(err)
	s.NotNil(originalUser)

	userData1 := *originalUser
	userData1.Username = "first_update"

	userData2 := *originalUser
	userData2.Username = "second_update"
...
func (s *RepositoryTestSuite) TestUpdateUser_EmailConfirmedNotChanged() {
	...
	// Обновляем username
	userData := *user
	userData.Username = "updated_username"
...
func (s *RepositoryTestSuite) TestUpdateUser_PasswordNotChanged() {
...
func (s *RepositoryTestSuite) TestGetUsers_EmailConfirmedFilter() {
