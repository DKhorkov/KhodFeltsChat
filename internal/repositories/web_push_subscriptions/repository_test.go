...
func (s *RepositoryTestSuite) TestCreateWebPushSubscription_Success() {
	...
	id, err := s.repository.CreateWebPushSubscription(s.ctx, subscription)
	s.NoError(err)
	s.NotZero(id)

	// Проверяем, что подписка была создана
	subscriptions, err := s.repository.GetWebPushSubscriptionsByUserID(s.ctx, 2)
	s.NoError(err)
...
func (s *RepositoryTestSuite) TestGetWebPushSubscriptionsByUserID_Success() {
	subscriptions, err := s.repository.GetWebPushSubscriptionsByUserID(s.ctx, 1)
	s.NoError(err)
...
func (s *RepositoryTestSuite) TestGetWebPushSubscriptionsByUserID_NoSubscriptions() {
	subscriptions, err := s.repository.GetWebPushSubscriptionsByUserID(s.ctx, 3)
	s.NoError(err)
	s.Empty(subscriptions)
}

func (s *RepositoryTestSuite) TestDeleteWebPushSubscription_Success() {
	...
	err = s.repository.DeleteWebPushSubscription(s.ctx, subscriptions[0].ID)
	s.NoError(err)

	// Проверяем, что осталась только одна подписка
	remaining, err := s.repository.GetWebPushSubscriptionsByUserID(s.ctx, 1)
	s.NoError(err)
	s.Len(remaining, 1)
}

func (s *RepositoryTestSuite) TestDeleteWebPushSubscription_NotFound() {
	// Удаление несуществующей подписки не возвращает ошибку
	err := s.repository.DeleteWebPushSubscription(s.ctx, 999)
	s.NoError(err)
}
