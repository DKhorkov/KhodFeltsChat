# repositories/web_push_subscriptions
...
| Метод | Описание |
|-------|----------|
| `CreateWebPushSubscription(ctx, subscription)` | Вставляет запись подписки, возвращает ID |
| `GetWebPushSubscriptionsByUserID(ctx, userID)` | Возвращает все подписки пользователя по его ID |
| `DeleteWebPushSubscription(ctx, id)` | Удаляет подписку по ID |
...
- `github.com/DKhorkov/kfc/internal/domains` — тип `WebPushSubscription`
