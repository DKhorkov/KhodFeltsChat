...
func TestNewTraceDecorator(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			base: mockrepositories.NewMockSettingsRepository(gomock.NewController(t)),
		},
		{
...
func TestTraceDecorator_CreateSettings(t *testing.T) {
	t.Parallel()

	tests := []struct {
		name          string
		settings      domains.Settings
		setupMocks    func(*mocktracing.MockProvider, *mockrepositories.MockSettingsRepository, *mocktracing.MockSpan)
		expectedError error
	}{
		{
			name:     "successful create settings with tracing",
			settings: domains.Settings{UserID: 1, Theme: domains.ThemeLight},
			setupMocks: func(
				mockProvider *mocktracing.MockProvider,
				mockBase *mockrepositories.MockSettingsRepository,
				...
				mockBase.EXPECT().
					CreateSettings(gomock.Any(), domains.Settings{UserID: 1, Theme: domains.ThemeLight}).
					Return(nil)
			},
			expectedError: nil,
		},
		{
			name:     "create settings error with tracing",
			settings: domains.Settings{UserID: 1, Theme: domains.ThemeLight},
			setupMocks: func(
				mockProvider *mocktracing.MockProvider,
				mockBase *mockrepositories.MockSettingsRepository,
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			mockBase := mockrepositories.NewMockSettingsRepository(ctrl)
			...
			err := decorator.CreateSettings(context.Background(), tt.settings)
...
func TestTraceDecorator_UpdateSettings(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			err := decorator.UpdateSettings(context.Background(), tt.settings)
