...
| Ошибка | Описание |
|---|---|
| `ErrMessageNotFound` | Сообщение не найдено |
| `ErrNotMessageAuthor` | Действие может выполнить только автор сообщения (удаление для всех, редактирование) |

### reactions.go — реакции

| Ошибка | Описание |
|---|---|
| `ErrReactionAlreadyExists` | Реакция уже поставлена этим юзером на это сообщение (дубликат ADD → 409) |
| `ErrReactionNotFound` | Реакция с указанным ID отсутствует в справочнике (→ 404) |
| `ErrReactionNotSet` | На сообщении нет реакции от этого юзера (REMOVE идемпотентен → 200, но broadcast не публикуется) |
