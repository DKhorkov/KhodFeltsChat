...
#### Автоматическая синхронизация браузерной подписки

При загрузке настроек с сервера, если `webPushConsents & ConsentNewMessage != 0`, но в текущем браузере нет активной push-подписки — фронтенд автоматически запрашивает разрешение и подписывается (`PushManager.subscribe`), чтобы серверное согласие было подкреплено браузерной подпиской. Если пользователь откажет в разрешении браузера — сбрасываем серверный бит.
